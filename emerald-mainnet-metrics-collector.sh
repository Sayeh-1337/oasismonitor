...
committeePeersCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".committee.peers | length' <<< $oasisControlStatus)
latestHeight=$(jq -r '.consensus.latest_height' <<< $oasisControlStatus)
latestEpoch=$(jq -r '.consensus.latest_epoch' <<< $oasisControlStatus)

runtimesLatestRound=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".latest_round' <<< $oasisControlStatus)
if [[ $runtimesLatestRound == null ]]; then runtimesLatestRound=0; fi
runtimesCommitteeLatestHeight=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".committee.latest_height' <<< $oasisControlStatus)
if [[ $runtimesCommitteeLatestHeight == null ]]; then runtimesCommitteeLatestHeight=0; fi
runtimesExecutorRolesCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".committee.executor_roles | length' <<< $oasisControlStatus)
runtimesStorageRolesCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".committee.storage_roles | length' <<< $oasisControlStatus)
runtimesStorageLastFinalizedRound=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e2eaa99fc008f87f".storage.last_finalized_round' <<< $oasisControlStatus)
