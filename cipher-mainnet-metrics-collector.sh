...
committeePeersCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".committee.peers | length' <<< $oasisControlStatus)
latestHeight=$(jq -r '.consensus.latest_height' <<< $oasisControlStatus)
latestEpoch=$(jq -r '.consensus.latest_epoch' <<< $oasisControlStatus)

runtimesLatestRound=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".latest_round' <<< $oasisControlStatus)
if [[ $runtimesLatestRound == null ]]; then runtimesLatestRound=0; fi
runtimesCommitteeLatestHeight=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".committee.latest_height' <<< $oasisControlStatus)
if [[ $runtimesCommitteeLatestHeight == null ]]; then runtimesCommitteeLatestHeight=0; fi
runtimesExecutorRolesCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".committee.executor_roles | length' <<< $oasisControlStatus)
runtimesStorageRolesCount=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".committee.storage_roles | length' <<< $oasisControlStatus)
runtimesStorageLastFinalizedRound=$(jq -r '.runtimes."000000000000000000000000000000000000000000000000e199119c992377cb".storage.last_finalized_round' <<< $oasisControlStatus)
