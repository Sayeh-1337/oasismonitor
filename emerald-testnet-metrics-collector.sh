...
committeePeersCount=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".committee.peers | length' <<< $oasisControlStatus)
latestHeight=$(jq -r '.consensus.latest_height' <<< $oasisControlStatus)
latestEpoch=$(jq -r '.consensus.latest_epoch' <<< $oasisControlStatus)

runtimesLatestRound=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".latest_round' <<< $oasisControlStatus)
if [[ $runtimesLatestRound == null ]]; then runtimesLatestRound=0; fi
runtimesCommitteeLatestHeight=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".committee.latest_height' <<< $oasisControlStatus)
if [[ $runtimesCommitteeLatestHeight == null ]]; then runtimesCommitteeLatestHeight=0; fi
runtimesExecutorRolesCount=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".committee.executor_roles | length' <<< $oasisControlStatus)
runtimesStorageRolesCount=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".committee.storage_roles | length' <<< $oasisControlStatus)
runtimesStorageLastFinalizedRound=$(jq -r '.runtimes."00000000000000000000000000000000000000000000000072c8215e60d5bca7".storage.last_finalized_round' <<< $oasisControlStatus)
